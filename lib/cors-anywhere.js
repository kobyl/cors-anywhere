...
function withCORS(headers, request) {
  ...
  headers['access-control-expose-headers'] = Object.keys(headers).join(',')+',kook';
    ...
    if (parsedLocation) {
      if (statusCode === 301 || statusCode === 302 || statusCode === 303) {
        ...
        if (requestState.redirectCount_ <= requestState.maxRedirects) {
          ...
          // And of course this swallows incoming cookies too, which is the whole point in using this.
          if(proxyRes.headers['set-cookie'])
            res.setHeader('kook', proxyRes.headers['set-cookie']);
...
// Such an ugly hack that it deserves a reward.
// We need the cookie but fetch silently strips set-cookie headers, 
// so translating into a random header to get around the limitation.
if(proxyRes.headers['set-cookie'])
    proxyRes.headers['kook'] = proxyRes.headers['set-cookie'];
...
function getHandler(options, proxy) {
  ...
  return function(req, res) {
    ...
    if(req.headers['kook']) {
      req.headers['cookie'] = req.headers['kook'];
    }
